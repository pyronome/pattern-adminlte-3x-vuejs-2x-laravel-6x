...
use App\Http\Requests\Wisilo\API\ProfilePOSTRequest;
...
class ProfileController extends Controller
{
    ...
    public function get(Request $request)
    {
        ...
        $userData = $objectWisilo->getUserData();

        $data = [];

        if ($userData['id'] > 0)
        {
            $objectWisiloUser = WisiloUser::find($userData['id']);
    ...
    public function get_files(Request $request) {
        ...
        if ($userData['id'] > 0)
        { 
            $files = $objectWisilo->get_model_files('WisiloUser', $userData['id']);
            $index = 0;
    ...
    public function post(ProfilePOSTRequest $request)
    {
        ...
        if ($objectWisiloUser != null)
        {
            ...
            if ('' != $request->input('password1')) {
                $objectWisiloUser->password = bcrypt($request->input('password1'));
            } 
    ...
    public function groupHasConfigParameter($id) {
        $configList = WisiloUserConfig::where('owner_group', $id)
            ->where('deleted', 0)
            ->where('enabled', 1)
            ->get();
    ...
    public function get_config_data(Request $request)
    {
        ...
        $objectId = $User->id;
        $owner_group = $User->wisilousergroup_id;
    ...
    public function getConfigObjectByKey($__key) {
        return WisiloUserConfig::where('__key', $__key)
            /* ->where('owner_group', $owner_group) */
            ->where('deleted', 0)
            /* ->where('enabled', 1) */
            ->first();
