...
class AdminLTELogController extends Controller
{
    public function get_recordlist(Request $request)
    {
        ...
        if ($filter_used) {
            $objectList = AdminLTELog::defaultQuery($search_text, $sort_variable, $sort_direction)->whereIn('id', $filteredIds)->paginate($limit, ['*'], 'page', $page);
        } else {
            $objectList = AdminLTELog::defaultQuery($search_text, $sort_variable, $sort_direction)->paginate($limit, ['*'], 'page', $page);
        }
        ...
        foreach ($objectList as $object) {
            ...
            $datetime_format = config('adminlte.date_format') . ' ' . config('adminlte.time_format');
    ...
    private function filtered_by_default($connection, $filter_data) {
        $ids = [];
        $baseSQL = 'SELECT DISTINCT id FROM adminltelogtable WHERE deleted=0';
        $filterSQL = '';
    ...
    private function get_user_display_texts() {
        $objectList = AdminLTEUser::where('deleted', 0)->get();
        $list = [];
    ...
    public function get_filter_options(Request $request) {    
        ...
        // User
        $objectList = AdminLTEUser::where('deleted', 0)->orderBy('fullname', 'asc')->get();
        $list = [];
        $index = 0;
        foreach ($objectList as $object) {
            $list[$index]['id'] = $object->id;
