...
class AdminLTEConfigPOSTRequest extends FormRequest
{
    /**
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            $currentObject = AdminLTEConfig::where('id', $this->id)->first();
    ...
    public function customValidator($currentObject, $validator) {
        ...
        $key = $objectAdminLTE->convertTitleToConfigName($this->title);

        if ('' != $parent_key) {
            $key = $parent_key . '.' . $key;
        }

        $anyObject = AdminLTEConfig::where('id', '!=', $this->id)->where('deleted', 0)->where('__key', '==', $key)->first();
