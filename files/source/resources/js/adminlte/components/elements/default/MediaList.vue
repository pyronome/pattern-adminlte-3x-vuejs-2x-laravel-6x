            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover table-sm">
                        <thead>
                            <tr>
                                <th class="text-center sbp-item" model-permission-token="adminltemedia-delete">
                                    <div class="icheck-primary d-inline">
                                        <input type="checkbox"
                                            @click="select_all_row($event.target)"
                                            id="select_AdminLTEMedia_rows"
                                            class="select_all_row"
                                            data-model="AdminLTEMedia">
                                        <label for="select_AdminLTEMedia_rows"></label>
                                    </div>
                                </th>
                                <th>
                                    <button type="button"
                                        id="'button_sort_AdminLTEMedia_id"
                                ...
                                <th>
                                    <button type="button"
                                        id="'button_sort_AdminLTEMedia_file_title"
                                        class="button-table-sort"
                                ...
                                <th>
                                    <button type="button"
                                        id="'button_sort_AdminLTEMedia_file_name"
                                        class="button-table-sort"
                                ...
                                <th>
                                    <button type="button"
                                        id="'button_sort_AdminLTEMedia_mime_type"
                                        class="button-table-sort"
                                ...
                                <th class="text-center th-btn-1">
                                    <button id="buttonNewAdminLTEMedia"
                                        @click="editMedia({})"
                        ...
                        <tbody id="tbodyAdminLTEMediaRecordList">
                            <tr v-for="row in list" :key="row.id">
                                <td class="text-center sbp-item" model-permission-token="adminltemedia-delete">
                                    <div class="icheck-primary d-inline">
                                        <input type="checkbox"
                                            @click="select_adminltemedia_row($event.target)"
                                            :id="'select_AdminLTEMedia_row' + row.id"
                                            class="select_adminltemedia_row"
                                            data-model="AdminLTEMedia"
                                            :data-row-id="row.id">
                                        <label :for="'select_AdminLTEMedia_row' + row.id"></label>
                                    </div>
                                </td>
                                <td v-html="row.id"></td>
                                ...
                                <td class="text-center">
                                    <button class="btn btn-outline-primary btn-xs btn-on-table sbp-item btnEditMedia"
                                        @click="editMedia(row)"
                                        menu-permission-token="adminltemedia"
                                        model-permission-token="adminltemedia-read">
    ...
    export default {
        ...
        methods: {
            ...
            editMedia: function(object) {
                ...
                document.getElementById("__media__guid").value = AdminLTEHelper.generateGUID("media");
            ...
            submitMediaForm: function() {
                ...
                axios.post( 
                        AdminLTEHelper.getAPIURL("adminltemedia/post_media"),
                        formData,
                        {
                            headers: {
            ...
            loadData: function (callback) {
                ...
                axios.get(AdminLTEHelper.getAPIURL("adminltemedia/get_recordlist/" + query))
                    .then(({ data }) => {
                        self.data = data;
            ...
            sort: function (variable) {
                AdminLTEHelper.activateSortLoader(`button_sort_AdminLTEMedia_${variable}`);
                ...
                var self = this;
                this.loadData(function() {
                    AdminLTEHelper.deactivateSortLoader(`button_sort_AdminLTEMedia_${self.sort_variable}`, self.sort_direction)
                });
            },
            select_all_row: function(sender) {
                ...
                $(".select_adminltemedia_row").prop("checked", sender.checked);
                this.updateCheckboxStates();
            },
            select_adminltemedia_row: function() {
                this.updateCheckboxStates();
            },
            updateCheckboxStates: function() {
                var checkboxCount = $(".select_adminltemedia_row").length;
                var selectedCount = $(".select_adminltemedia_row:checked").length;
                
                if (0 == selectedCount) {
                    $("#select_AdminLTEMedia_rows").prop("checked", false);
                    
                    $("#buttonDeleteAdminLTEMedia").hide();
                    $("#buttonNewAdminLTEMedia").show();
                } else {
                    $("#buttonDeleteAdminLTEMedia > .selected-count").html(selectedCount);
                    
                    $("#buttonNewAdminLTEMedia").hide();
                    $("#buttonDeleteAdminLTEMedia").show();

                    if (selectedCount == checkboxCount) {
                        $("#select_AdminLTEMedia_rows").prop("checked", true);
                    } else {
                        $("#select_AdminLTEMedia_rows").prop("checked", false);
                    }
                }
            ...
            cleanCheckedBoxes: function() {
                $(".select_adminltemedia_row").prop("checked", false);
                this.updateCheckboxStates();
            },
            deleteSelectedRows: function() {
                var selectedCheckboxes = $(".select_adminltemedia_row:checked");
                var selectedIds = [];
                ...
                /* Vue.swal.fire({
                    ...
                    if (result.isConfirmed) {
                        self.formDelete.selected_ids = AdminLTEHelper.getTableSelectedRowIds("AdminLTEMedia");
            ...
            submitDeleteForm: function () {
                var self = this;
                self.$Progress.start();
                self.formDelete.post(AdminLTEHelper.getAPIURL("adminltemedia/delete_files"))
                    .then(({ data }) => {
                        self.$Progress.finish();
        ...
        mounted() {
            ...
            this.loadData(function(){
                AdminLTEHelper.setDefaultSortButton("button_sort_AdminLTEMedia_id");
            });
        },
        updated() {
